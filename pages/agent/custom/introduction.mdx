...
Custom agent skills are a powerful feature of Qubex LLM, but they can also be dangerous if misused.
...
Qubex LLM allows you to create custom agent skills that can be used to extend the capabilities of your `@agent` invocations. These skills can be anything you want from a simple API call to even operating-system invocations.

The sky is the limit! Depending on how you run Qubex LLM, you can create custom agent skills that can run extra processes like running a local Python script or, on Desktop, even operating-system invocations.

If it can be done in NodeJS, it can likely be done in Qubex LLM.

## The current state of custom agent skills

<Callout type="info" emoji="️💡">
 Custom agent skills are newly supported in Qubex LLM and may have some bugs, quirks, missing features, unsupported features, etc.
...
4. There is currently no established tooling for creating custom agent skills - so follow this guide if developing skills for Qubex LLM.
...
Custom agent skills are available in Qubex LLM Desktop version **1.6.5 and later.**

Custom agent skills are **not** available in the Qubex LLM Cloud offering.

## View loaded custom agent skills

You can view the loaded custom agent skills by opening the `Agent Skills` tab in the settings of Qubex LLM.

Any valid custom agent skills loaded into Qubex LLM will be displayed here.
