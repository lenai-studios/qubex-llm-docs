...
description: "Learn how to enable feature previews of Qubex LLM"
---
...
While we take great care to craft features carefully we also want to proactively offer non-standard features to users of Qubex LLM when in development or when we are looking for feedback.
...
- Partial or full data loss of Qubex LLM's local database, source files, stored documents, or datastores
...
- Unhandled bugs, exceptions, and crashes of Qubex LLM

# How to enable feature previews

<Callout>
  If you follow this procedure and nothing happens then this means that there
  are no active previews available for your version of Qubex LLM.
</Callout>

To enable feature previews in Qubex LLM in _any_ form (Docker, Desktop, Hosted) open the settings page by clicking on the "wrench" icon on the left sidebar.
