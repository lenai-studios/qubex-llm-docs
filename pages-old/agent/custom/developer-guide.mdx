...
**This guide is intended for developers who want to create custom agent skills for Qubex LLM.**
...
1. NodeJS 18+
2. Yarn
3. Qubex LLM running in some supported environment see [here](./introduction.mdx) for more information.
...
Qubex LLM supports hot loading of custom agent skills. This means that you can make changes to your custom agent skill and see the changes without having to restart the agent or the instance of Qubex LLM.

## Where to place your custom agent skill code

All agents skills must be placed in the appropriate folder in your Qubex LLM storage directory folder. This can be found in multiple locations depending on the environment you are running Qubex LLM in.
...
When running Qubex LLM locally, your storage directory is likely mounted in the `server/storage` directory.
...
When running Qubex LLM on Desktop, your storage directory can be [found using this guide](../../installation-desktop/storage.mdx#where-is-my-data-located).
