...
description: "Local Docker Installation guide for Qubex LLM"
---
...
<Image
  ...
  width={1920}
  quality={100}
  alt="Qubex LLM Installation Local Docker"
/>

# How to use Dockerized Anything LLM

Use the Dockerized version of Qubex LLM for a much faster and complete startup of Qubex LLM.

## Minimum Requirements

<Callout type="info" emoji="️💡">
  **Tip** ➤➤ Running Qubex LLM on AWS/GCP/Azure?
...
➤ Qubex LLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

➤ Qubex LLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

</Callout>

## Recommend way to run dockerized Qubex LLM!
...
the docker container using Qubex LLM as `localhost:xxxx` will not resolve for the host system
...
➤ eg: Chroma host URL running on localhost:8000 on host machine needs to be `http://host.docker.internal:8000` when used in Qubex LLM.
...
```shell copy
 docker pull mintplexlabs/Qubex LLM
```
...
Mount the storage locally and run Qubex LLM in Docker

<Tabs items={['Linux/Mac', 'Windows']} defaultIndex="0">
  <Tabs.Tab>
  ```shell copy showLineNumbers
  export STORAGE_LOCATION=$HOME/Qubex LLM && \
  ...
  mintplexlabs/Qubex LLM
  ```
 </Tabs.Tab>

 <Tabs.Tab>
  ```powershell copy showLineNumbers
  $env:STORAGE_LOCATION="$HOME\Documents\Qubex LLM"; `
...
Go to `http://localhost:3001` and you are now using Qubex LLM! All your data and progress will persist between
...
- `touch server/storage/Qubex LLM.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Qubex LLM easier.

### Use the Midori AI Subsystem to Manage Qubex LLM
...
After setting that up, install the Qubex LLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to `http://127.0.0.1:11434` you should put `http://host.docker.internal:11434` into the connection URL in Qubex LLM.
